...
4. **Read `docs/PRODUCT_BLUEPRINT.md`** to understand what's built and what's planned.

## Step 2: Deep Inspection

1. **Scope the inspection** - read every file in the directories this feature touches. Follow import chains 2 levels deep from the entry point. Read the SQLAlchemy models for every table this feature queries. Do NOT claim you "inspected the codebase" after reading 3 files.
...
1. **What exists today that this touches?** Files, routes, models, endpoints. Cite line numbers.
...
6. **What dependencies or prerequisites exist?** Model changes, other specs, config changes.
7. **What existing logic could this conflict with?** Shared routes, shared models, shared utilities.
8. **What is the end-to-end data flow?** User action -> API endpoint -> DB write -> response. No gaps.
9. **What is the correct implementation order?** Model first? Route first? Template first? Be explicit.
10. **What are the exact success criteria?** These become the builder's verification steps.
11. **What are the non-negotiable constraints?** Auth, data safety, privacy boundary, error codes.
