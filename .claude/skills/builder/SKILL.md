...
3. **Read relevant constraint files** in `.constraints/` for the domain you're touching.
...
Run `py -m compileall app/ -q`. If it fails, fix it NOW before touching another file. Do not accumulate errors. Catch regressions the moment they happen, not at the end.
...
1. **Compilation check:** `py -m compileall app tests main.py stats.py -q`. Paste output. Must exit 0.
2. **Test suite:** `py -3.12 -B -m pytest tests/ -x -q`. Paste output. Must exit 0. Skip if `.multi-agent-lock` exists.
3. **Manual verification:** If the feature has a UI or API endpoint, start the server and test it. Show the response or describe what you observed.
4. **Edge cases tested:** List each edge case from the spec. For each: what you did, what happened (not what "would" happen).
5. **Regression check:** List every module that imports or shares code with your changes. Verify at least one still works.
6. **Before vs after:** Show what changed. Test output, API responses, or behavioral comparison.
...
**Self-enforcement:** You must run every verification step and the Final Check ON YOUR OWN before telling the developer the build is complete. If you say "done" without pasted compilation output, test output, and verification evidence, you have failed the gate.
